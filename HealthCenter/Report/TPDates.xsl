...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" 	xmlns:fo="http://www.w3.org/1999/XSL/Format">	
		...
		<xsl:template match="/TPDates">	
			<fo:root>
			
				<fo:layout-master-set>
					<fo:simple-page-master master-name="TPDates" page-width="21.5cm"
				...
				<fo:page-sequence master-reference="TPDates">
	    ...
	    <!-- Header Test -->
			<fo:block>
				<fo:block font-size="14pt" font-weight="bold" text-align="center">Appointments with Treatment Plan Due Report</fo:block>
		...
		<xsl:template match = "Table">
			...
			<xsl:variable name="Break" select='TableBody/Appointments/Appointment/Resource'/>
			
				<fo:block padding-top="1cm" font-size="10pt" font-weight="normal">
					<fo:table table-layout="fixed" width="100%" padding-bottom="1cm" margin-left="1.5cm">
		...
		<xsl:template match = "TableBody">
				...
				<xsl:choose>
					<xsl:when test='Resource=Previous'>
						<fo:table-row>		
							<fo:table-cell>
