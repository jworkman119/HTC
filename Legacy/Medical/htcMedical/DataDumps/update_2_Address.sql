...
--patients that have changed address
select distinct cast(datadump.account as bigint) as account, datadump.zipcode
from datadump
	left join address on datadump.account = address.patient_account
where  datadump.zipcode <> address.zip
union
-- new patients
select distinct datadump.account, datadump.zipcode
from datadump
where datadump.account not in(
								select address.patient_account
								from address
								)
order by account
...
-- finding last zip code, for people with multiple entries
...
-- setting current address for people that have multiple addresses
update Address 
Set CurrentAddress = 1
from Address
...
--setting old Address to not current
update Address
Set CurrentAddress = 0
from address
	inner join #tblTemp on Address.Patient_Account = #tblTemp.Account
where #tblTemp.ZipCode <> Address.Zip  
