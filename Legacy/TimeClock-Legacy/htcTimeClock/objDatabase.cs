...
namespace htcTimeClock
{
    class objDatabase
    {
        ...
        public void UpdateDB(int intID)
        {
            ...
            SQLiteConnection sqlConnection = CreateSQLite_Connection();
            strWorker = getWorker(intID, sqlConnection);
            strStatus = getStatus(intID, strTime,ref InID, sqlConnection);
            if (strStatus != "Double-Swipe")
                updateLocalDB(intID,strTime, strStatus,sqlConnection);
        ...
        private SQLiteConnection CreateSQLite_Connection()
        {
            ...
            SQLiteConnection objConnection = new SQLiteConnection(@"Data Source= c:\Program Files\HTC\StateFair\timeclock.sqlite");
